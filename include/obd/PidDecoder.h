...
public:
    ...
    static float32 decodeEngineLoad(uint8 const A) { return A * 100.0f / 255.0f; }
    ...
    static float32 decodeTemperature(uint8 const A) { return static_cast<float32>(A) - 40.0f; }
    ...
    static float32 decodeMafFlow(uint8 const A, uint8 const B) { return ((A << 8) | B) / 100.0f; }
    ...
    static float32 decodePercentage(uint8 const A) { return A * 100.0f / 255.0f; }
    ...
    // ...
    static float32 decodeCatalystTemp(uint8 const A, uint8 const B) { return ((A << 8) | B) / 10.0f - 40.0f; }

    // 0x42 - Control Module Voltage (V)
    static float32 decodeControlModuleVoltage(uint8 const A, uint8 const B) { return ((A << 8) | B) / 1000.0f; }

    // 0x43 - Absolute Load Value (%)
    static float32 decodeAbsoluteLoad(uint8 const A, uint8 const B) { return ((A << 8) | B) * 100.0f / 255.0f; }

    // 0x44 - Commanded Equivalence Ratio (lambda)
    static float32 decodeCommandedEquivRatio(uint8 const A, uint8 const B) { return ((A << 8) | B) / 32768.0f; }

    // 0x50 - Maximum value for Fuel Flow Analysis (g/s)
    static float32 decodeMaxMafFlow(uint8 const A) { return A * 10.0f; }

    // 0x53 - Absolute Evap System Vapor Pressure (kPa)
    static float32 decodeAbsEvapVaporPressure(uint8 const A, uint8 const B) { return ((A << 8) | B) / 200.0f; }

    // 0x54 - Evap System Vapor Pressure (Pa)
    static float32 decodeEvapVaporPressureAlt(uint8 const A, uint8 const B) { return ((A << 8) | B) - 32767.0f; }

    // 0x5D - Fuel Injection Timing (°)
    static float32 decodeFuelInjectionTiming(uint8 const A, uint8 const B) { return ((A << 8) | B) / 128.0f - 210.0f; }

    // 0x5E - Engine Fuel Rate (L/h)
    static float32 decodeEngineFuelRate(uint8 const A, uint8 const B) { return ((A << 8) | B) / 20.0f; }
